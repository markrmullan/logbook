...
final class LogbookClientHandlerSpringWebServerTest {
    ...
    LogbookClientHandlerSpringWebServerTest() {
        server.start();
    ...
    void shouldLogRequestWithBody() throws IOException, InterruptedException {
        ...
        assertThat(message, startsWith("Incoming Request:"));
        assertThat(message, containsString(format("POST http://localhost:%d/ HTTP/1.1", server.getPort())));
